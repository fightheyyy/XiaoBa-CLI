...
本文维护 XiaoBa-CLI 仓库级执行计划。`docs/SPEC.md` 定义项目级架构和 contract，本文维护当前状态、下一步、验收条件和验证证据。五个顶层模块的状态继续放在各自的小 `PLAN.md`；更细的 durable 子模块可以继续维护自己的 `SPEC.md` / `PLAN.md`。

## Current Status

XiaoBa-CLI 当前是一套本地优先、message-native 的 agent harness runtime。CLI、Feishu、Weixin、Pet、Dashboard 等入口都收敛到同一套核心 runtime；roles 和 skills 提供行为策略；session log、data、memory 和 output 构成当前证据与状态资产；benchmarks 和 tests 是回归反馈入口。文档治理口径现在固定为 `docs/SPEC.md` 加五个顶层模块小 spec：Surfaces、Harness Runtime、Roles & Skills、State & Evidence、Evaluation Gates。
...
- `docs/SPEC.md` 定义 harness 边界、三层状态模型、message-native runtime、logging/evidence、evaluation 和 hard contracts。
- 五个顶层模块 spec/plan 已建立或对齐：`docs/surfaces/`、`docs/harness/`、`docs/roles/`、`docs/state-evidence/`、`docs/benchmarks/`。
...
- 文档治理已补齐 `docs/PLAN.md` 和主要 SPEC 的 Current/Target 架构图，但后续实现改动仍需持续维护。
...
- The five top-level module specs exist and stay discoverable from `docs/SPEC.md`: `docs/surfaces/SPEC.md`, `docs/harness/SPEC.md`, `docs/roles/SPEC.md`, `docs/state-evidence/SPEC.md`, and `docs/benchmarks/SPEC.md`.
...
- 2026-05-29：Created `docs/PLAN.md` and aligned `docs/SPEC.md` with Current/Target architecture diagrams using module-level nodes.
...
- 2026-05-30：Aligned top-level governance to one docs spec plus five module specs; added missing Surfaces, Harness Runtime, and State/Evidence module SPEC/PLAN files and linked all five from `docs/SPEC.md`.
...
- 2026-05-30：Reduced duplicate documentation roots: `docs/SPEC.md` / `docs/PLAN.md` are now the project-level source of truth, root `SPEC.md` / `PLAN.md` were removed, and conceptual module docs live under `docs/` instead of root-only folders.
