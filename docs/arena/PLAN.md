...
15. M14：Cat effectiveness data slice：completed for dev + holdout + broad holdout live slices. 7 SkillsBench-derived cases are materialized locally with workspace fixtures, subject skill, hidden oracle/verifier and expected labels.
...
- Expand Cat / Arena effectiveness from first broad holdout to stability proof: rerun the current 7 SkillsBench-derived gold cases across seeds / providers / time windows, track false pass / false blocking / timeout / cost, and only generalize beyond the current case family after repeated samples hold.
...
- Cat effectiveness data：`arena/benchmarks/cat-effectiveness/cases/**` for curated SkillsBench-derived gold cases; source provenance is embedded in each case manifest, and `sources/` is only a placeholder until the fuller source catalog moves to Barena. Generated `runs/**` scorecards remain local evidence by default
...
- 2026-07-01：Added the Cat effectiveness technical report. `docs/arena/CAT_EFFECTIVENESS_REPORT.md` now documents proof levels, experiment design, current evidence, what is and is not proven, and the next live SkillsBench proof path. Verification：`node --test -r tsx test/arena-cat-effectiveness-data.test.ts test/arena-cat-effectiveness-scorer.test.ts`；`npm run build`；`git diff --check -- docs/arena/CAT_EFFECTIVENESS_REPORT.md docs/arena/CAT_EFFECTIVENESS_SPEC.md docs/arena/PLAN.md`。
- 2026-07-01：Implemented the deterministic Arena Cat effectiveness scorer. `src/arena/cat-effectiveness.ts` loads SkillsBench-derived gold labels, scores UserCat / InspectorCat / ReviewerCat independently, writes `cat-effectiveness-scorecard.json`, and marks Reviewer false pass / UserCat oracle leakage as invalid loop failures. Verification：`node --test -r tsx test/arena-cat-effectiveness-data.test.ts test/arena-cat-effectiveness-scorer.test.ts`；`npm run build`。
- 2026-06-30：Seeded the first SkillsBench-derived Arena Cat effectiveness cases: executable-priority `skillsbench.offer-letter-generator.v1`, second-seed `skillsbench.citation-check.v1`, and focused contract tests. Source provenance is stored in each case manifest; `sources/` remains a placeholder until the fuller source catalog moves to Barena. `.gitignore` now allows curated `arena/benchmarks/**` sources/cases while keeping generated `runs/**` ignored. Verification：`node --test -r tsx test/arena-cat-effectiveness-data.test.ts`；`git diff --check -- .gitignore arena/benchmarks/cat-effectiveness test/arena-cat-effectiveness-data.test.ts docs/arena/CAT_EFFECTIVENESS_SPEC.md docs/arena/SPEC.md docs/arena/PLAN.md`。
