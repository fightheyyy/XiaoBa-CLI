...
它是一个 **活在 IM 里的 AI 同事**：一个可以长期待在聊天里的角色。它能理解群聊 / 私聊上下文，接收文件和任务，调用工具；在 `engineer-cat` 角色下可以调度 Codex / Claude Code 等外部 coding agent 到后台干活，并把进度和产物发回工作开始的地方。
...
IM 消息 / CLI 输入
  ...
  -> 电脑 / 文件 / 项目 / shell / AutoDev
  -> EngineerCat 可调度 Codex / Claude Code
...
第一步先从实用场景切入：**活在 IM 里的工作同事**。它能从聊天里接任务、调用工具；其中 EngineerCat 可以调度 Codex 或 Claude Code，并回到聊天里汇报进度、文件和证据。
...
XiaoBa 不试图替代 Codex、Claude Code 或其他 coding agent。它把这些工具放在 EngineerCat 的工程边界内调度，再由运行证据、ReviewerCat 和用户反馈来判断、整合和验收。
