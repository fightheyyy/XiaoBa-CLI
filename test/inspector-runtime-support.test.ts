...
describe('InspectorRuntimeSupport', () => {
  ...
  test('可以在独立 runtime 里收日志并自动分析', async () => {
    ...
    const reviewExecutor: InspectorReviewExecutor = {
      async reviewCase(record, caseStore) {
        ...
        fs.writeFileSync(handoffPath, JSON.stringify({
          ...
          nextState: 'fixing',
          summary: 'runtime hook evidence is actionable',
          evidenceSummary: {
            rootCauseHypothesis: 'runtime tool path failed',
            confidence: 'medium',
            signals: ['tool failure'],
          },
    ...
    assert.strictEqual(result.result.summary.handoffGenerated, true);
  });
    ...
    const reviewExecutor: InspectorReviewExecutor = {
      async reviewCase(record, caseStore) {
        ...
        fs.writeFileSync(handoffPath, JSON.stringify({
          ...
          nextState: 'blocked',
          summary: 'sample is too thin for production routing',
          evidenceSummary: {
            rootCauseHypothesis: 'insufficient signal',
            confidence: 'high',
            signals: ['only one uploaded log line'],
          },
          labels: ['runtime', 'inspector'],
        }, null, 2), 'utf-8');
        return {
