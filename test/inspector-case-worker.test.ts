  ...
  test('会自动处理 received case，并生成分析结果', async () => {
    ...
    const reviewExecutor: InspectorReviewExecutor = {
      async reviewCase(record, caseStore) {
        ...
        const handoffPath = path.join(caseDir, 'inspector-handoff.json');
        ...
        fs.writeFileSync(handoffPath, JSON.stringify({
          ...
          title: 'runtime command mismatch',
          category: 'runtime_bug',
          priority: 'high',
          recommendedNextAction: 'runtime_fix',
          nextState: 'fixing',
          summary: 'platform command mismatch routed to EngineerCat',
          evidenceSummary: {
            rootCauseHypothesis: 'shell command was not portable',
            confidence: 'high',
            signals: ['head is not available on this platform'],
          },
        ...
        return {
          ...
          handoffFilePath: 'inspector-handoff.json',
          finalText: '已发送审查摘要',
          deliveries: [
            { type: 'text', text: '摘要' },
    ...
    assert.strictEqual(result.summary.handoffGenerated, true);
    assert.strictEqual(result.deliveries.length, 2);
    assert.deepStrictEqual(result.artifact_manifest.map((item: any) => [item.path, item.action, item.metadata?.source]), [
      ['agent-review.md', 'generated', 'inspector_hook'],
      ['inspector-handoff.json', 'generated', 'inspector_hook'],
      ['agent-review.md', 'sent', 'inspector_hook'],
    ]);
    assert.ok(fs.existsSync(path.join(store.getCaseDir(created.caseId), 'agent-review.md')));
    assert.ok(fs.existsSync(path.join(store.getCaseDir(created.caseId), 'inspector-handoff.json')));
