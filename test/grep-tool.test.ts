...
function toolOutputText(output: unknown): string {
  ...
  if (output && typeof output === 'object' && 'toolContent' in output) {
    return String((output as ToolExecutionOutput).toolContent);
  }
...
describe('GrepTool', () => {
  ...
  beforeEach(() => {
    ...
    const rawExecute = grepTool.execute.bind(grepTool);
    (grepTool as any).execute = async (...args: Parameters<GrepTool['execute']>) => toolOutputText(await rawExecute(...args));
  });

  describe('基础搜索功能', () => {
    test('runtime-facing execute 应返回结构化 ToolExecutionOutput', async () => {
      ...
      assert.strictEqual((result as ToolExecutionOutput).status, 'success');
      assert.match(String((result as ToolExecutionOutput).toolContent), /test1\.js/);
    });
  ...
  describe('过滤功能', () => {
    test('应该支持glob过滤', async () => {
      const result = await (grepTool as any).executeWithNodeJS({
        pattern: 'Hello',
        glob: '*.js',
        output_mode: 'files'
      }, testDir, context);
