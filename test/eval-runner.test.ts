  ...
  test('delivery evidence contract fails when surface runtime has no structured delivery evidence', async () => {
    ...
    fs.writeFileSync(fixturePath, `${JSON.stringify({
      ...
      surface_runtime: {
        ...
        status_code: 200,
        session_key: 'pet:room:agent-runtime',
  ...
  test('surface runtime verifier fails when required file delivery evidence is missing', async () => {
    ...
    fs.writeFileSync(fixturePath, `${JSON.stringify({
      ...
      surface_runtime: {
        ...
        path: '/api/room/agents/:agentId/message',
        status_code: 200,
        session_key: 'pet:room:engineer-cat',
      ...
      cases: [
        {
          ...
          hard_verifiers: [
            { id: 'jsonl_parse' },
            {
              id: 'surface_runtime_e2e',
              config: {
                expected_surface: 'dashboard',
                expected_runtime_id: 'dashboard_room_router',
                expected_status_code: 200,
                min_visible_deliveries: 1,
                min_file_deliveries: 1,
                required_files: ['dashboard-runtime-file-report.md'],
