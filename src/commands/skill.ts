...
export function registerSkillCommand(program: Command): void {
  const addRoleOption = <T extends Command>(command: T): T =>
    command.option('-r, --role <name>', '使用指定角色（roles/<name>）');

  const skillCmd = addRoleOption(program
    .command('skill')
    .description('管理 XiaoBa skills'));

  // skill list - 列出所有可用的 skills
  addRoleOption(skillCmd
    .command('list')
...
async function listSkills(): Promise<void> {
  ...
  if (skills.length === 0) {
    ...
    Logger.info('\n提示：');
    Logger.info(`  - 将 skill 放在 ${styles.code(PathResolver.getSkillsPath())} 目录`);
