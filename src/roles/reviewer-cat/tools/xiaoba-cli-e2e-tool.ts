...
function renderReport(input: {
  ...
  const lines = [
    '# XiaoBa-CLI 真实端到端验收中文报告',
    '',
    `Run ID：${input.manifest.runId}`,
    `目标角色：${input.manifest.targetRole}`,
    `交互入口：${input.manifest.surface}`,
    `验收结论：${input.manifest.decision}`,
    `评分：${input.manifest.score}`,
    input.manifest.blockedReason ? `阻塞原因：${input.manifest.blockedReason}` : undefined,
    input.manifest.fallbackReason ? `降级原因：${input.manifest.fallbackReason}` : undefined,
    '',
    '## 真实交互',
    '',
    `交互入口：${input.manifest.surface}`,
    input.manifest.tmuxSession ? `Tmux 会话：${input.manifest.tmuxSession}` : undefined,
    input.manifest.processPid ? `进程 PID：${input.manifest.processPid}` : undefined,
    `启动命令：${input.manifest.command}`,
    '',
    '用户消息：',
    ...
    `原始终端记录：${input.scorecard.evidence.rawPane}`,
    `清洗后终端记录：${input.scorecard.evidence.cleanPane}`,
    '',
    'Session logs：',
    ...(input.manifest.sessionLogPaths.length > 0 ? input.manifest.sessionLogPaths.map(item => `- ${item}`) : ['- 未发现']),
    '',
    '## 三层证据',
    '',
    ...
    '## 角色有效性评分',
    '',
    `评级：${input.scorecard.roleEffectiveness.rating}`,
    `总分：${input.scorecard.roleEffectiveness.totalScore}/${input.scorecard.roleEffectiveness.maximumScore}`,
    '',
    ...input.scorecard.roleEffectiveness.dimensions.map((dimension: any) => [
      `- ${dimension.id}: ${dimension.score}/${dimension.maxScore}`,
      `  要求：${dimension.requirement}`,
      `  证据：${Array.isArray(dimension.evidence) && dimension.evidence.length > 0 ? dimension.evidence.join(' | ') : '无'}`,
    ].join('\n')),
    '',
    '## 错误',
    '',
    ...(input.errors.length > 0 ? input.errors.map(item => `- ${item}`) : ['- 无']),
    '',
    '## 剩余风险',
    '',
    ...(input.scorecard.residualRisks.length > 0 ? input.scorecard.residualRisks.map((item: string) => `- ${item}`) : ['- 无']),
...
function renderThreeLayerEvidence(summary: Record<string, any>): string[] {
  const layers = [
    ['Durable Session / 可恢复会话', summary.durableSession],
    ['Working Trace / 工作 trace', summary.workingTrace],
    ['Provider Transcript / Provider 转录', summary.providerTranscript],
  ];
  return [
    ...layers.map(([label, value]) => [
      `- ${label}: ${value?.status || 'missing'}`,
      `  证据：${Array.isArray(value?.evidence) && value.evidence.length > 0 ? value.evidence.join(' | ') : '无'}`,
      `  备注：${Array.isArray(value?.notes) && value.notes.length > 0 ? value.notes.join(' | ') : '无'}`,
    ].join('\n')),
    '',
    '问题：',
    ...(Array.isArray(summary.issues) && summary.issues.length > 0 ? summary.issues.map((item: string) => `- ${item}`) : ['- 无']),
  ];
}
...
function collectSessionLogs(cwd: string, sinceMs: number): string[] {
  ...
  const visit = (dir: string) => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entryPath.replace(/\\/g, '/').includes('/context-snapshots/')) continue;
