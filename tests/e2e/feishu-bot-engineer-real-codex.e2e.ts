...
describe('FeishuBot Engineer real Codex E2E', () => {
  after(() => {
    ...
    fs.rmSync(path.join(process.cwd(), 'data', 'sessions', 'user_ou_feishu_bot_real_codex_user.jsonl'), { force: true });
    fs.rmSync(path.join(process.cwd(), 'data', 'sessions', 'user_ou_feishu_bot_edit_codex_user.jsonl'), { force: true });
  });
